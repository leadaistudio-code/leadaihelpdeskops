...
import { FolderClock, Plus, Filter } from "lucide-react";

export default async function ActiveIncidentsPage() {
  const session = await getServerSession(authOptions);
  
  // Fetch all incidents and filter by active status
    ...
    <div className="p-8 h-full overflow-auto custom-scrollbar relative z-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 mt-4 gap-6">
        <div className="flex items-center space-x-4">
          <div className="w-12 h-12 rounded-xl bg-sky-500/20 flex items-center justify-center">
            <FolderClock className="w-6 h-6 text-sky-400" />
          </div>
          <div>
            <h1 className="text-3xl font-extrabold text-white tracking-tight">Open Incidents</h1>
            <p className="text-slate-400 mt-1">All active tickets requiring attention</p>
          </div>
        </div>
      ...
      <div className="glass-panel rounded-3xl overflow-hidden border border-white/10">
        <div className="px-8 py-6 border-b border-white/5 bg-slate-900/50">
          <h2 className="text-sm font-black text-slate-300 uppercase tracking-widest">Active Queue</h2>
        </div>
