...
const CSS = `
.mc-root{
  --ink:#080B11; --ink-2:#0B0F17; --panel:#0F141D; --panel-2:#141B26;
  --line:#1F2937; --line-soft:#161E29; --text:#EAEEF5; --muted:#8D98AC;
  --dimtext:#5C6678; --signal:#38E8B0; --signal-deep:#12B489; --signal-dim:#1E7D63;
  --violet:#6D5EF6; --warn:#F5B14C; --crit:#FF6B7D;
...
.mc-root h1,.mc-root h2,.mc-root h3{margin:0;letter-spacing:-0.03em;font-weight:800;text-wrap:balance;line-height:1.04}
.mc-root .wrap{max-width:1160px;margin:0 auto;padding:0 28px}
.mc-root .eyebrow{font-family:var(--mono);font-size:12px;letter-spacing:.22em;text-transform:uppercase;color:var(--signal);font-weight:600}
.mc-root .btn{display:inline-flex;align-items:center;gap:9px;font-weight:700;font-size:14px;border-radius:11px;padding:14px 22px;cursor:pointer;border:1px solid transparent;transition:transform .18s cubic-bezier(.2,.7,.3,1),box-shadow .25s,background .2s}
.mc-root .btn:active{transform:translateY(1px) scale(.99)}
.mc-root .btn-primary{background:linear-gradient(135deg,var(--signal),var(--signal-deep));color:#03130d;box-shadow:0 8px 30px rgba(56,232,176,.28)}
.mc-root .btn-primary:hover{box-shadow:0 12px 44px rgba(56,232,176,.44);transform:translateY(-2px)}
.mc-root .btn-ghost{background:rgba(255,255,255,.03);border-color:var(--line);color:var(--text)}
.mc-root .btn-ghost:hover{border-color:var(--signal-dim);background:rgba(56,232,176,.06)}
.mc-root .arrow{transition:transform .2s}
.mc-root .btn:hover .arrow{transform:translateX(4px)}

.mc-root nav{position:sticky;top:0;z-index:50;backdrop-filter:blur(14px);background:rgba(8,11,17,.62);border-bottom:1px solid var(--line-soft)}
...
.mc-root .logo{width:32px;height:32px;border-radius:9px;display:grid;place-items:center;background:linear-gradient(135deg,var(--signal),#25a8d0);color:#04120d;font-weight:900;font-family:var(--mono);box-shadow:0 0 22px rgba(56,232,176,.4)}
...
.mc-root .hero{position:relative;padding:96px 0 40px;overflow:hidden}
.mc-root #field{position:absolute;inset:0;width:100%;height:100%;z-index:0}
.mc-root .hero-glow{position:absolute;z-index:1;pointer-events:none}
.mc-root .glow-a{top:-120px;right:-80px;width:640px;height:640px;background:radial-gradient(circle,rgba(56,232,176,.14),transparent 62%);filter:blur(20px)}
.mc-root .glow-b{bottom:-200px;left:-120px;width:620px;height:620px;background:radial-gradient(circle,rgba(109,94,246,.16),transparent 60%);filter:blur(24px)}
...
.mc-root .badge{display:inline-flex;align-items:center;gap:9px;padding:7px 14px;border:1px solid var(--line);border-radius:999px;background:rgba(255,255,255,.02);font-family:var(--mono);font-size:11.5px;letter-spacing:.12em;text-transform:uppercase;color:var(--muted);font-weight:600}
.mc-root .dotlive{position:relative;width:7px;height:7px}
.mc-root .dotlive::before{content:"";position:absolute;inset:0;border-radius:50%;background:var(--signal);animation:mc-ping 1.8s cubic-bezier(0,0,.2,1) infinite}
.mc-root .dotlive::after{content:"";position:absolute;inset:0;border-radius:50%;background:var(--signal)}
@keyframes mc-ping{75%,100%{transform:scale(2.6);opacity:0}}
.mc-root h1.hero-h{font-size:clamp(38px,6vw,66px);margin:26px 0 22px}
.mc-root h1.hero-h .accent{background:linear-gradient(120deg,var(--signal),#7ce0ff);-webkit-background-clip:text;background-clip:text;color:transparent}
.mc-root .hero-sub{font-size:19px;line-height:1.55;color:var(--muted);max-width:540px;margin-bottom:32px}
...
.mc-root .trust b{color:var(--muted);font-weight:600}
.mc-root .trust .sep{width:4px;height:4px;border-radius:50%;background:var(--line)}

.mc-root .tiltwrap{perspective:1400px}
.mc-root .status{transform-style:preserve-3d;transition:transform .3s cubic-bezier(.2,.7,.3,1);border-radius:20px;border:1px solid var(--line);background:linear-gradient(180deg,rgba(20,27,38,.9),rgba(11,15,23,.92));box-shadow:0 30px 80px -20px rgba(0,0,0,.7);overflow:hidden}
.mc-root .status .bar{display:flex;align-items:center;gap:8px;padding:13px 16px;border-bottom:1px solid var(--line-soft);background:rgba(255,255,255,.015)}
...
.mc-root .status .big{font-size:34px;font-weight:800;letter-spacing:-.02em;font-variant-numeric:tabular-nums;margin:2px 0 18px}
.mc-root .status .big .u{font-size:15px;color:var(--signal);font-weight:600;margin-left:4px}
.mc-root .meter{height:7px;border-radius:99px;background:var(--panel-2);overflow:hidden;margin-bottom:16px}
.mc-root .meter i{display:block;height:100%;border-radius:99px;background:linear-gradient(90deg,var(--signal-deep),var(--signal));width:0;transition:width 1.4s cubic-bezier(.2,.7,.3,1)}
.mc-root .healthgrid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:14px 0 6px}
.mc-root .hg{border:1px solid var(--line-soft);border-radius:10px;padding:10px;text-align:center;background:rgba(255,255,255,.01)}
.mc-root .hg .v{font-size:18px;font-weight:800;font-variant-numeric:tabular-nums}
...
.mc-root .heal{display:flex;align-items:center;gap:10px;margin-top:16px;padding:11px 13px;border-radius:11px;background:rgba(56,232,176,.07);border:1px solid var(--signal-dim)}
.mc-root .heal .ic{width:26px;height:26px;border-radius:7px;background:var(--signal);color:#03130d;display:grid;place-items:center;font-weight:900;font-size:13px;flex-shrink:0}
...
.mc-root h2.big-h{font-size:clamp(28px,4.4vw,44px);margin-bottom:16px}
.mc-root .lede{font-size:18px;color:var(--muted);max-width:620px;line-height:1.55}
.mc-root .center .lede{margin:0 auto}

.mc-root .demoframe{max-width:1000px;margin:0 auto;border-radius:14px;overflow:hidden;border:1px solid var(--line);box-shadow:0 40px 100px -30px rgba(0,0,0,.8)}
.mc-root .demoframe .top{height:44px;background:var(--panel);display:flex;align-items:center;padding:0 16px;gap:7px;border-bottom:1px solid var(--line-soft)}
...
.mc-root .logos{border-top:1px solid var(--line-soft);border-bottom:1px solid var(--line-soft);padding:30px 0}
...
.mc-root .logorow span{font-size:18px;font-weight:800;letter-spacing:-.02em;color:var(--muted);opacity:.62;transition:opacity .2s}
...
.mc-root .panel{border:1px solid var(--line-soft);border-radius:18px;padding:28px;background:linear-gradient(180deg,var(--panel-2),var(--panel))}
.mc-root .panel.bad{border-color:rgba(255,107,125,.2)}
...
.mc-root .panel.bad .mk{background:rgba(255,107,125,.12);color:var(--crit)}
.mc-root .panel.good .mk{background:rgba(56,232,176,.12);color:var(--signal)}
.mc-root .panel.good li{color:var(--text)}
...
.mc-root .cap{transform-style:preserve-3d;transition:transform .25s cubic-bezier(.2,.7,.3,1),border-color .25s,box-shadow .25s;border:1px solid var(--line-soft);border-radius:16px;padding:24px;background:linear-gradient(180deg,var(--panel-2),var(--panel));height:100%;position:relative}
.mc-root .cap:hover{border-color:var(--signal-dim);box-shadow:0 24px 60px -24px rgba(56,232,176,.28)}
.mc-root .cap .ic{width:42px;height:42px;border-radius:11px;display:grid;place-items:center;background:rgba(56,232,176,.09);border:1px solid var(--signal-dim);margin-bottom:16px;transform:translateZ(30px)}
.mc-root .cap .ic svg{width:21px;height:21px;stroke:var(--signal)}
.mc-root .cap h3{font-size:16.5px;margin-bottom:8px;transform:translateZ(18px)}
...
.mc-root .step .node{width:52px;height:52px;border-radius:14px;display:grid;place-items:center;border:1px solid var(--signal-dim);background:rgba(56,232,176,.07);margin-bottom:18px;box-shadow:0 0 30px rgba(56,232,176,.14)}
.mc-root .step .node svg{width:24px;height:24px;stroke:var(--signal)}
.mc-root .step h3{font-size:18px;margin-bottom:8px}
.mc-root .step p{font-size:14px;color:var(--muted);line-height:1.55;margin:0}

.mc-root .roi{border:1px solid var(--line);border-radius:24px;overflow:hidden;position:relative;background:linear-gradient(135deg,rgba(56,232,176,.06),rgba(109,94,246,.05)),var(--ink-2)}
...
.mc-root .stat{border:1px solid var(--line-soft);border-radius:16px;padding:22px;background:rgba(8,11,17,.5)}
.mc-root .stat .v{font-size:40px;font-weight:800;letter-spacing:-.03em;font-variant-numeric:tabular-nums;line-height:1}
...
.mc-root .price{display:flex;flex-direction:column;border:1px solid var(--line-soft);border-radius:18px;padding:28px;background:linear-gradient(180deg,var(--panel-2),var(--panel))}
.mc-root .price.pop{border-color:var(--signal-dim);box-shadow:0 24px 70px -30px rgba(56,232,176,.4);position:relative}
.mc-root .price.pop::before{content:"Most popular";position:absolute;top:-11px;left:50%;transform:translateX(-50%);font-family:var(--mono);font-size:10px;letter-spacing:.12em;text-transform:uppercase;color:#03130d;background:var(--signal);padding:4px 12px;border-radius:999px;font-weight:700}
...
.mc-root .price .amt{font-size:44px;font-weight:800;letter-spacing:-.03em;font-variant-numeric:tabular-nums;line-height:1}
...
.mc-root .price li{display:flex;gap:10px;padding:8px 0;font-size:13.5px;color:var(--text)}
...
.mc-root .quote blockquote{font-size:clamp(22px,3.2vw,30px);font-weight:700;letter-spacing:-.02em;line-height:1.35;margin:0 0 24px}
...
.mc-root .final .box{border:1px solid var(--signal-dim);border-radius:24px;padding:56px 40px;background:radial-gradient(600px 300px at 50% 0%,rgba(56,232,176,.1),transparent 60%),var(--ink-2)}
.mc-root .final h2{font-size:clamp(30px,4.6vw,46px);margin-bottom:16px}
...
.mc-root footer{border-top:1px solid var(--line-soft);padding:52px 0 40px;margin-top:20px}
...
export default function LandingPage() {
  ...
  return (
    <div className="mc-root" ref={root}>
      ...
      <section className="sec" id="heal" style={{ background: "linear-gradient(180deg,transparent,rgba(56,232,176,.03),transparent)" }}>
        <div className="wrap">
